...
def unificar_capa_oro(tipo="movies"):
    ...
    try:
        ...
        # (Esto asegura que no se pierdan películas o series no presentes en tendencias de Trakt)
        ...
        if "espectadores_live" in df_final.columns:
            df_final["espectadores_live"] = (
                df_final["espectadores_live"].fillna(0).astype(int)
            )
        ...
        print(
            f" No hay archivo Trakt limpio para {tipo}. Creando dataset solo con TMDB."
        )
        df_final = df_base

    # Limpieza de columnas duplicadas post-merge
    if "titulo_x" in df_final.columns:
        df_final = df_final.rename(columns={"titulo_x": "titulo"}).drop(
            columns=["titulo_y"], errors="ignore"
        )
...
def preparar_matriz_ia():
    ...
    try:
        ...
        df_ia = pd.merge(
            df_ratings, df_links[["movieId", "tmdbId"]], on="movieId", how="inner"
        )
