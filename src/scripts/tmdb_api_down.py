...
def descargar_series_por_mes(ano_inicio=1990, ano_fin=2025):
    print(
        "Iniciando extracción masiva de series mes a mes (Tolerancia a fallos: Activada)..."
    )
    ...
    with open(ruta_archivo, "a", encoding="utf-8") as archivo:
        for ano in range(ano_inicio, ano_fin + 1):
            for mes in range(1, 13):
                ...
                while (
                    pagina_actual <= 500
                ):  # Ahora es matemáticamente imposible llegar a 500
                    ...
                    if respuesta.status_code == 200:
                        ...
                        print(
                            "Límite de API. Esperando 10 segundos para descongestionar..."
                        )
                        time.sleep(10)

                    else:
                        print(
                            f"Error HTTP {respuesta.status_code} en página {pagina_actual}. Saltando..."
                        )
                        break
